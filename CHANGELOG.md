...
- Production builds now emit one directly-openable `dist/index.html`: Vite's IIFE
  output folds imported CSS into the classic script, then the build plugin
  moves that script after the app root and inlines it. The file can be opened
  directly without a local server; the build fails if future code splitting or
  emitted assets would silently break that guarantee.

- "Add to a channel" now offers a "Known agent" picker alongside the free-text pubkey field: once a channel is chosen, it lists every agent this app has an attestation for that isn't already a member of that channel (by name, via `AgentRow.channelNames` -- already computed for the Members directory, reused rather than a new query). Picking one fills the pubkey field and the select resets to its own placeholder; it's a one-shot shortcut for the common case, not a second piece of state to keep in sync with the field, and typing an arbitrary pubkey by hand still works exactly as before for a target this app has never seen an attestation for. New i18n keys (`membership.knownAgentLabel` and three option strings) added to all six locales.

- Reworked the session backfill subscription into two phases, because kind:0 (profile) was competing for the same shared `BACKFILL_LIMIT` as every kind this app actually cares about, even though most profiles a busy relay ever serves belong to pubkeys with nothing to do with this community. `session.ts` now asks for the structural kinds (channels, membership, audit log) first, bounded by a renamed `STRUCTURAL_BACKFILL_LIMIT` (same value, 500, as before -- kept there deliberately, see its docblock: a page-based table doesn't get more useful to a human past a few hundred rows regardless of community size). Once that subscription's EOSE reports which pubkeys actually showed up (via a new `pubkeysIn` helper reading validated mutations, never raw event tags), a second subscription asks for exactly those authors' profiles plus a permanent unscoped `limit: 0` filter, so a profile from any pubkey -- including one never seen in the structural phase -- is still caught live afterward. Narrowing scope only helps profile, though: membership and audit history have no "noise" to narrow away, so a new `onHistoryTruncated` callback fires once if the structural backfill's event count hits its cap, surfaced in `CommunityPanel` as its own hint (`community.historyMayBeIncomplete`) -- deliberately not folded into the existing relay-`notice` channel, since that phrase means "the relay said X" and this is this client's own inference from an event count. True `until`-cursor pagination would remove the ceiling entirely; deferred, not overlooked, along with batching `ingest()`'s currently one-write-per-event backfill processing -- both are a fuller management-interface's problem to solve, not this page's.
