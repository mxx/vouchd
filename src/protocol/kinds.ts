...
/**
 * NIP-29 group member snapshot: the relay's own signed roster for one
 * channel, addressable by `d` = channel id, with one `["p", pubkey, "",
 * role]` tag per active member. Regenerated by the relay on every
 * membership change (buzz-relay's emit_group_discovery_events ->
 * store_group_members_event), which makes it the only *authoritative*
 * statement of who is in a channel -- replaying add/remove events only ever
 * reconstructs the subset this client happened to observe.
 *
 * Confirmed via buzz-core kind.rs (KIND_NIP29_GROUP_MEMBERS = 39002).
 */
